...
(function (TL) {
  'use strict';
  const def = TL.defGlyph, lerp = TL.geom.lerp;
  const J = 'join', T = 'term', H = 'h';

  /* ---------- shared constructions ---------- */

  /* zig-zag of diagonals (A V W M v w): apex sharpness comes from m.apex */
  function zig(g, m, xs, yT, yB, startTop, o) {
    const a = m.apex, n = xs.length, Hh = yT - yB, ys = [];
    for (let i = 0; i < n; i++) {
    ...
    const cmds = [['M', xs[0], ys[0]]];
    for (let i = 1; i < n; i++) cmds.push(['L', xs[i], ys[i], ys[i] > ys[i - 1] ? { w: 'thin' } : {}]);
    const e = a > 0.03 ? 0 : m.os;
    g.path(cmds, Object.assign({ s: H, e: H, part: 'diagonal', miter: 18, serifS: 'both', serifE: 'both',
      clip: Object.assign({ y0: yB - e, y1: yT + e }, o && o.clipX) }, o));
    return ys;
  }

  /* bowl attached to a stem with straight top/bottom (B D P R) */
  function capBowl(g, m, x0, yT, yB, R) {
  ...
  function branchBowl(g, m, xStem, xFar, yT, yB, o) {
    ...
    g.path([['M', xStem, yT - jh], ['vh', cx, yT], ['hv', xFar, cy], ['vh', cx, yB], ['hv', xStem, yB + jh]],
      Object.assign({ s: J, e: J, ws: 0.6, we: 0.6, part: 'bowl', counter: true }, o));
  }

  function oval(g, m, xl, xr, yb, yt, o) {
    const cx = (xl + xr) / 2, cy = (yb + yt) / 2;
    g.path([['M', cx, yt], ['hv', xr, cy], ['vh', cx, yb], ['hv', xl, cy], ['vh', cx, yt], ['Z']], Object.assign({ part: 'bowl' }, o));
  }

  /* open round shape (C c G e): returns nothing, draws from top terminal round to bottom */
  function openBowl(g, m, xl, xr, yb, yt, u0, u1, o) {
    const cx = (xl + xr) / 2, cy = (yb + yt) / 2;
    g.path([['M', xr, cy], ['vh', cx, yt, { u0 }], ['hv', xl, cy], ['vh', cx, yb], ['hv', xr, cy, { u1 }]],
      Object.assign({ s: T, e: T, part: 'bowl' }, o));
  }

  function sShape(g, m, x0, W, yB, yT, o) {
    const sc = (o && o.scale) || 1, hs = m.s * sc / 2, hh = m.hT * sc / 2, Hh = yT - yB;
  ...
  const mapCmds = (cmds, f) => cmds.map(c => {
    if (c[0] === 'Z') return c;
    const o = c.slice(); for (let i = 1; i + 1 < c.length && typeof c[i] === 'number'; i += 2) { const p = f(c[i], c[i + 1]); o[i] = p[0]; o[i + 1] = p[1]; }
    return o;
  });
  ...
  const lc = m => ({ X: m.xh, hs: m.s / 2, hh: m.hT / 2, yt: m.xh + m.os - m.hT / 2, yb: -m.os + m.hT / 2 });
  ...
  function six(g, m, flip) {
    ...
    const f = flip ? (x, y) => [W - x, C - y] : (x, y) => [x, y];
